...
export function stipendio(player) {
  ...
  const scontoU21 = player.gruppo === "U21" ? 0.5 : 1;
  return fasciaStipendio(player.costo) * Math.pow(1.1, player.rinnovi || 0) * quota * scontoU21;
}
...
// Impatto economico di un singolo prestito giocatore, diviso in due parti:
// - "fisso": quello che paghi/incassi comunque, a prescindere da un eventuale riscatto (Costo prestito + Stipendio)
// - "variabile": il Costo riscatto — conta nel totale solo con Obbligo (certo), mai con Secco/Diritto finché non lo eserciti
export function impattoFissoPrestito(p) {
  const stipendioEffettivo = (p.stipendioACarico || 0) * ((p.quotaPercento ?? 100) / 100);
  return (p.costoPrestito || 0) + stipendioEffettivo;
}
export function impattoPrestitoGiocatore(p) {
  const riscattoEffettivo = p.tipo === "Obbligo di riscatto" ? (p.costoRiscatto || 0) : 0;
  return impattoFissoPrestito(p) + riscattoEffettivo;
}
