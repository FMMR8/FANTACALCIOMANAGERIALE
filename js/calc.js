...
export function calcolaContoEconomico(stato) {
  ...
  const ricPrestitiGiocatori = (stato.prestitiGiocatori || []).reduce((s, p) => {
    const stipendioEffettivo = (p.stipendioACarico || 0) * ((p.quotaPercento ?? 100) / 100);
    // Il Costo riscatto conta SOLO se l'Obbligo è certo — con Secco non c'entra, con Diritto
    // non è detto che si eserciti (conterà solo quando/se registri l'Acquisto vero a fine anno).
    const riscattoEffettivo = p.tipo === "Obbligo di riscatto" ? (p.costoRiscatto || 0) : 0;
    return s + (p.costoPrestito || 0) + riscattoEffettivo + stipendioEffettivo;
  }, 0);
