...
export const PREMIO_CHAMPIONS  = {
  "1° posto": 50, "2° posto": 45, "3°/4° posto": 40, "Non raggiunta": 35,
};
export const SANZIONI = [
  { da: 1,   a: 199,      punti: -3 },
  { da: 200, a: 499,      punti: -5 },
  { da: 500, a: Infinity, punti: -10 },
];

// --- Fasce stipendio / ammortamento (Cap. 5 e 12) ---
export function fasciaStipendio(costo) {
  if (costo <= 5) return 1;
  if (costo <= 10) return 2;
  if (costo <= 20) return 3;
  if (costo <= 30) return 4;
  if (costo <= 40) return 5;
  if (costo <= 50) return 6;
  if (costo <= 60) return 7;
  if (costo <= 70) return 8;
  if (costo <= 80) return 9;
  if (costo <= 90) return 10;
  if (costo <= 100) return 11;
  if (costo <= 150) return 12;
  return 13;
}
...
export function ammortamentoAnnuo(player, stagioneCorrente) {
  ...
  if (anni <= 0) return 0;
  const quota = player.quotaStagione ?? 1;
  return (player.costo / anni) * quota;
}
