...
function renderMercato() {
  ...
  <div class="sezione">
    ...
    <p style="font-size:12px;color:var(--gesso-ombra)">Indennizzo 0 se nessuno (il caso più comune), negativo se lo paghi tu. Il giocatore esce dalla Rosa in automatico.</p>
    <div class="griglia-3">
      <div class="campo"><label>Giocatore</label><select id="s-giocatore" onchange="document.getElementById('s-indennizzo').value = this.value === '' ? 0 : ${JSON.stringify(stato.rosa.map(p => calc.valoreResiduo(p, stato.stagioneCorrente).toFixed(1)))}[this.value]">${opzGiocatoriRosa()}</select></div>
...
async function renderRosaAltraSquadra(nickname, nomeVisualizzato) {
  ...
  const sezioni = gruppi.map(({ chiave, etichetta }) => {
    ...
    <div class="tabella-scroll"><table class="tabella-rosa">
      <thead><tr><th>Ruolo</th><th>Nome</th><th style="text-align:right">Anni rimanenti</th></tr></thead>
