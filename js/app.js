...
async function renderAltreSquadre() {
  ...
  // Tutte le sezioni si basano SOLO su stagioni già terminate (storico definitivo),
  // mai sui dati in corso — che cambierebbero di continuo e sarebbero fuorvianti.
  const conStorico = tutte.filter(s => (s.storicoStagioni || []).length > 0);

  if (conStorico.length === 0) {
    el.innerHTML = `
      <div class="sezione">
        <h3>Palmares della lega</h3>
        <p style="font-size:13px;color:var(--testo-soft);margin:0">
          Qui appariranno classifiche e albo d'oro dalla seconda stagione, quando le squadre inizieranno a terminare la prima.
        </p>
      </div>`;
    return;
  }
  ...
  // ---- 1) Classifica sportiva della stagione appena conclusa ----
  ...
  const righeSportiva = sportiva.map(({ nome, nickname, r }) => {
    const mia = nickname === nickCorrente;
    return `<tr${mia ? ' style="background:rgba(209,163,44,0.10)"' : ""}>
      <td>${r.piazzamentoCampionato ? r.piazzamentoCampionato + "°" : "—"}</td>
      <td>${nome}${mia ? " <span style='color:var(--oro-chiaro);font-size:11px'>(tu)</span>" : ""}</td>
      <td>${r.risultatoChampions || "—"}</td>
      <td>${r.piazzamentoCoppa ? r.piazzamentoCoppa + "°" : "—"}</td>
    </tr>`;
  }).join("");

  // ---- 2) Classifica finanziaria della stagione appena conclusa ----
  ...
  // ---- 3) Albo d'oro: chi ha vinto cosa, stagione per stagione ----
  const anni = [...new Set(conStorico.flatMap(s => s.storicoStagioni.map(h => h.anno)))].sort((a, b) => b - a);
  const vincitoriPerAnno = anni.map(anno => {
    const partecipanti = conStorico
      .map(s => { const r = rigaDi(s, anno); return r ? { nome: nomeDi(s), r } : null; })
      .filter(Boolean);
    return {
      anno,
      campionato: partecipanti.find(p => p.r.piazzamentoCampionato === 1)?.nome || null,
      coppa: partecipanti.find(p => p.r.piazzamentoCoppa === 1)?.nome || null,
      champions: partecipanti.find(p => p.r.risultatoChampions === "1° posto")?.nome || null,
    };
  });

  const righeAlbo = vincitoriPerAnno.map(v => `
    <tr>
      <td>${formattaStagione(v.anno)}</td>
      <td>${v.campionato || "—"}</td>
      <td>${v.champions || "—"}</td>
      <td>${v.coppa || "—"}</td>
    </tr>`).join("");

  // Conteggio trofei per squadra
  const trofei = {};
  vincitoriPerAnno.forEach(v => {
    [["campionato", v.campionato], ["coppa", v.coppa], ["champions", v.champions]].forEach(([tipo, nome]) => {
      if (!nome) return;
      if (!trofei[nome]) trofei[nome] = { campionato: 0, coppa: 0, champions: 0 };
      trofei[nome][tipo] += 1;
    });
  });
  const classificaTrofei = Object.entries(trofei)
    .map(([nome, t]) => ({ nome, ...t, totale: t.campionato + t.coppa + t.champions }))
    .sort((a, b) => b.totale - a.totale || b.campionato - a.campionato);

  // ---- 4) Migliori gestioni: le stagioni con la variazione di capitale più alta di sempre ----
  const tutteLeVariazioni = [];
  conStorico.forEach(s => {
    (s.storicoStagioni || []).forEach(h => {
      tutteLeVariazioni.push({ nome: nomeDi(s), nickname: s.nickname, anno: h.anno, variaz: h.variazioneCapitale });
    });
  });
  const migliori = tutteLeVariazioni.sort((a, b) => b.variaz - a.variaz).slice(0, 8);
  const righeMigliori = migliori.map((m, i) => {
    const mia = m.nickname === nickCorrente;
    const colore = m.variaz >= 0 ? "var(--ok)" : "var(--rosso-cartellino)";
    return `<tr${mia ? ' style="background:rgba(209,163,44,0.10)"' : ""}>
      <td>${i + 1}°</td>
      <td>${m.nome}${mia ? " <span style='color:var(--oro-chiaro);font-size:11px'>(tu)</span>" : ""}</td>
      <td>${formattaStagione(m.anno)}</td>
      <td class="cifra" style="color:${colore}">${m.variaz >= 0 ? "+" : ""}${m.variaz.toFixed(1)}</td>
    </tr>`;
  }).join("");

  const righeTrofei = classificaTrofei.map(t => `
    <tr>
      <td>${t.nome}</td>
      <td class="cifra">${t.campionato}</td>
      <td class="cifra">${t.champions}</td>
      <td class="cifra">${t.coppa}</td>
      <td class="cifra" style="color:var(--oro-chiaro);font-weight:700">${t.totale}</td>
    </tr>`).join("");

  el.innerHTML = `
    <div class="sezione">
      <h3>Classifica ${formattaStagione(annoUltimo)}</h3>
      <p style="font-size:12px;color:var(--testo-soft)">Come è finita la stagione.${notaMancanti}</p>
      <div class="tabella-scroll"><table class="tabella-rosa">
    ...
    <div class="sezione">
      ...
      <div class="tabella-scroll"><table class="tabella-rosa">
        ...
        <tbody>${righeAlbo}</tbody>
      </table></div>
    </div>

    ${classificaTrofei.length > 0 ? `
    <div class="sezione">
      <h3>Trofei vinti</h3>
      <p style="font-size:12px;color:var(--testo-soft)">Totale di tutte le stagioni giocate finora.</p>
      <div class="tabella-scroll"><table class="tabella-rosa">
        <thead><tr><th>Squadra</th><th>Camp.</th><th>CL</th><th>Coppa</th><th>Totale</th></tr></thead>
        <tbody>${righeTrofei}</tbody>
      </table></div>
    </div>` : ""}

    <div class="sezione">
      <h3>Migliori gestioni</h3>
      <p style="font-size:12px;color:var(--testo-soft)">Le stagioni con la crescita di capitale più alta di sempre.</p>
      <div class="tabella-scroll"><table class="tabella-rosa">
        <thead><tr><th>#</th><th>Squadra</th><th>Stagione</th><th>Variazione</th></tr></thead>
        <tbody>${righeMigliori}</tbody>
      </table></div>
    </div>`;
}
