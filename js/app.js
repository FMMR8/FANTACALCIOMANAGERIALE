...
function graficoLinee(serie, { altezza = 150, conLegenda = false } = {}) {
  ...
  const L = 52, R = 14, T = 14, B = 34;      // margini
  const W = 640, H = altezza;
  ...
  const tacche = Array.from({ length: 4 }, (_, i) => minY + (i / 3) * (maxY - minY));
  const griglia = tacche.map(v => `
    ...
    <text x="${L - 8}" y="${(py(v) + 4).toFixed(1)}" text-anchor="end"
          font-size="11" fill="#8a8f9a" font-family="monospace">${v.toFixed(0)}</text>`).join("");
  ...
  const etichetteX = anni.map((a, i) => {
    ...
    return `<text x="${px(a).toFixed(1)}" y="${H - 12}" text-anchor="${ancora}" font-size="11" fill="#8a8f9a">${formattaStagione(a)}</text>`;
  }).join("");
  ...
  const linee = serie.map(s => {
    ...
    const punti = ordinati.map(p => `<circle cx="${px(p.x).toFixed(1)}" cy="${py(p.y).toFixed(1)}" r="3.5" fill="${s.colore}" />`).join("");
    return `<path d="${d}" fill="none" stroke="${s.colore}" stroke-width="2.2" stroke-linejoin="round" stroke-linecap="round" />${punti}`;
  }).join("");
  ...
  return `<div class="tabella-scroll">
    <svg viewBox="0 0 ${W} ${H}" style="width:100%;min-width:420px;height:auto;display:block">
  ...
  const bloccoFinanze = `
    ...
    ${(() => {
      ...
      return `<div class="sezione">
        ...
        ${graficoLinee(serie, { altezza: 175, conLegenda: true })}
      </div>`;
    })()}
