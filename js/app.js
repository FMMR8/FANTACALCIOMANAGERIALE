import * as calc from "./calc.js?v=202609091126";
import {
...
} from "./storage.js?v=202609091126";
...
function statoVuoto(nickname, pin) {
  return {
    ...
    storicoStagioni: [], registroModifiche: [], archivioTrasferimenti: [],
  };
}
...
function graficoLinee(serie, { altezza = null, conLegenda = false } = {}) {
  ...
  serie = serie.map(s => ({
    ...s,
    punti: (s.punti || []).filter(p => Number.isFinite(p.x) && Number.isFinite(p.y)),
...
function renderStoricoStagioni() {
  ...
  const righe = storico.map(s => `
    <tr>
      <td>${s.anno}</td>
      <td class="cifra">${s.variazioneCapitale >= 0 ? "+" : ""}${s.variazioneCapitale.toFixed(1)}</td>
...
async function renderAltreSquadre() {
  ...
  const sportiva = conStorico
    ...
    .filter(Boolean)
    .sort((a, b) => (a.r.piazzamentoCampionato || 99) - (b.r.piazzamentoCampionato || 99));
  const finanziaria = sportiva.slice().sort((a, b) => b.r.capitaleProvvisorio - a.r.capitaleProvvisorio);
  const righeFinanziaria = finanziaria.map(({ nome, nickname, r }, i) => {
    return `<tr${rigaMia(nickname)}>
      <td>${i + 1}°</td>
      <td>${nome}${mioTag(nickname)}</td>
      <td class="cifra">${r.capitaleProvvisorio.toFixed(1)}</td>
    </tr>`;
  }).join("");

  const perVariazione = sportiva.slice().sort((a, b) => b.r.variazioneCapitale - a.r.variazioneCapitale);
  const righeVariazione = perVariazione.map(({ nome, nickname, r }, i) => {
    const colore = r.variazioneCapitale >= 0 ? "var(--ok)" : "var(--rosso-cartellino)";
    return `<tr${rigaMia(nickname)}>
      <td>${i + 1}°</td>
      <td>${nome}${mioTag(nickname)}</td>
      <td class="cifra" style="color:${colore}">${r.variazioneCapitale >= 0 ? "+" : ""}${r.variazioneCapitale.toFixed(1)}</td>
    </tr>`;
  }).join("");
  ...
  const archCapitale = squadreAnnoFin.slice()
    .sort((a, b) => b.r.capitaleProvvisorio - a.r.capitaleProvvisorio)
  ...
  const archGuadagni = squadreAnnoFin.slice()
    .sort((a, b) => b.r.variazioneCapitale - a.r.variazioneCapitale)
    .map(({ nome, nickname, r }, i) => {
      const colore = r.variazioneCapitale >= 0 ? "var(--ok)" : "var(--rosso-cartellino)";
      return `<tr${rigaMia(nickname)}>
        <td>${i + 1}°</td>
  ...
  const topAsta = tuttiTrasferimenti.filter(t => t.tipo === "asta").sort((a, b) => b.costo - a.costo).slice(0, 10);
  const topMercato = tuttiTrasferimenti.filter(t => t.tipo === "mercato").sort((a, b) => b.costo - a.costo).slice(0, 10);

  const righeAsta = topAsta.map((t, i) => `
    <tr${rigaMia(t.nickname)}>
      <td>${i + 1}°</td>
      <td>${t.nome || "—"}</td>
      <td class="cifra" style="color:var(--oro-chiaro);font-weight:700">${t.costo.toFixed(1)}</td>
...
async function completaTrasferimentoInterno(id, proposta) {
  ...
  // Se il mio lato risulta già completato, questa è una ripetizione: non applico nulla.
  if (sonoProponente ? proposta.completatoDaProponente : proposta.completatoDaProprietario) return;
