import * as calc from "./calc.js?v=202609100041";
import {
...
} from "./storage.js?v=202609100041";
...
async function confermaVotiOscar() {
  ...
  try {
    await salvaVotoOscar(legaCorrente, stato.stagioneCorrente, nickCorrente, votiOscarCorrenti);
  } catch (e) {
...
function bloccoOscar(voti, tutte, conStorico) {
  ...
  if (mancanti > 0) {
    return `${selettore}
      <div class="sezione">
        <p style="font-size:13px;color:var(--testo-soft);margin:0">
          Votazioni della stagione ${formattaStagione(annoMostrato)} ancora in corso: hanno votato
  ...
  return `${selettore}<div class="affiancate due">${podi}</div>`;
}
...
async function renderAltreSquadre() {
  ...
  if (atabAttiva === "oscar") {
    ...
    let voti = [];
    try { voti = await elencoVotiOscar(legaCorrente); }
...
function renderIstruzioni() {
  ...
  ${sezioneIstr("📁 Come funziona il sito", `
    ...
    <b>Cosa trovi dentro.</b> Sette schede in alto: <b>Anagrafica</b> (dati società e bacheca trofei), <b>Stadio</b>, <b>Sponsor & Premi</b>, <b>Rosa</b> (il riepilogo, sola lettura), <b>Gestione Rosa</b> (acquisti, svincoli, rinnovi), <b>Banca</b>, <b>Conto Economico</b> (dove si chiude la stagione). Sopra, due pulsanti: <b>🔄 Mercato</b> per trattare con le altre squadre, e <b>🏆 Albo d'oro</b> per titoli, classifiche e record di tutta la lega.<br><br>
