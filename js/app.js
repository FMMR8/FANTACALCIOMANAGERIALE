...
function renderAnagrafica() {
  ...
  <div class="sezione">
    ...
    <div class="griglia-2">
      ...
      <div class="campo">
        <label>Stagione corrente (anno) — stagione ${stato.stagioneCorrente}/${String(stato.stagioneCorrente + 1).slice(-2)}</label>
...
function renderRosa() {
  ...
  <div class="sezione">
    ...
    <p style="font-size:12px;color:var(--gesso-ombra)">
      Questa pagina è un <b>report</b>: mostra chi hai in squadra, ma non si modifica direttamente da qui.
      Per far entrare o uscire un giocatore vai su <b>Mercato</b> (Acquisti, Cessioni, Svincoli) — anche i rinnovi di contratto si fanno da lì, non qui.
      Questo evita che i dati di un contratto (anni, costo) cambino per sbaglio a stagione in corso.<br><br>
      ...
      Poi ci sono fino a 7 slot Under 21, e infine slot Extra senza limite — sono separati da una riga colorata nella tabella.
      Quota %: 100 se il giocatore è tuo per tutta la stagione, 50 se entra/esce a metà stagione, 0 se è fuori in prestito.
    </p>
    ...
    <div class="tabella-scroll">
      <table class="tabella-rosa">
        <thead><tr>
          ...
          <th>Anni rim.</th><th>Stipendio</th><th>Quota %</th><th>Ammort.</th><th>Rinnovi</th><th>Val. residuo</th>
        </tr></thead>
  ...
  ${renderRegistroModifiche()}`;
}

function separatoreRosa(testo) {
  return `<tr><td colspan="11" style="background:var(--ambra); color:var(--blu-scuro); font-weight:600; font-size:12px; text-align:center; padding:6px;">${testo}</td></tr>`;
}
...
function renderRegistroModifiche() {
  ...
  <div class="sezione">
    ...
    <p style="font-size:12px;color:var(--gesso-ombra)">Ogni acquisto, cessione, svincolo e rinnovo viene registrato qui con data e ora — visibile anche all'ADMIN aprendo la squadra, per controllare che nessuna modifica sospetta sia stata fatta a stagione in corso.</p>
    <details ${log.length > 0 ? "" : "style='display:none'"}>
      <summary>Vedi registro (${log.length} movimenti)</summary>
    ...
    ${log.length === 0 ? '<p style="font-size:12px;color:var(--gesso-ombra)">Ancora nessun movimento registrato.</p>' : ""}
  </div>`;
}
...
function renderMercato() {
  ...
  <div class="sezione">
    ...
    <p style="font-size:12px;color:var(--gesso-ombra)">Il giocatore finisce automaticamente in Rosa non appena lo aggiungi qui — non serve inserirlo due volte.</p>
    <div class="griglia-3">
    ...
    <button id="btn-agg-asta" class="btn-piccolo">+ Aggiungi (va in Rosa)</button>
  ...
  <div class="sezione">
    ...
    <p style="font-size:12px;color:var(--gesso-ombra)">Anche questi finiscono automaticamente in Rosa.</p>
    <div class="griglia-3">
  ...
  <div class="sezione">
    ...
    <p style="font-size:12px;color:var(--gesso-ombra)">Qui sotto la lista di chi ha il contratto in scadenza <b>questa stagione</b> (Anni rimanenti = 0). Spunta chi vuoi rinnovare, scegli il nuovo anno di fine per ognuno, poi conferma — si aggiorna la Rosa e lo stipendio (+10%) in un colpo solo. Non si fa più modificando a mano in Rosa.</p>
  ...
  <div class="sezione">
    ...
    <p style="font-size:12px;color:var(--gesso-ombra)">
      Se lo prendi IN ENTRATA non aggiungerlo in Rosa con un costo vero (o mettilo a 0): il vero impatto lo scrivi qui.<br>
      <b>Secco</b>: valorizza solo "Costo prestito" (0 se gratis), lascia "Costo riscatto" a 0.<br>
      <b>Diritto/Obbligo di riscatto</b>: valorizza sia "Costo prestito" sia "Costo riscatto" (0 se gratis) — i costi possono essere sempre 0 se volete.<br>
      <b>Stipendio a carico</b> funziona come in Rosa: importo annuo × quota (100/50/0%) in base a quanta stagione copre.<br>
      Quando il riscatto/obbligo si esercita davvero a fine anno, il giocatore diventa tuo per sempre: registra un Acquisto (in asta o fuori asta) con costo = costo riscatto, e rimuovi la riga da qui. Con un prestito Secco invece il giocatore torna al proprietario da solo, non serve fare nulla.
    </p>
    <div class="tabella-scroll">
      <table class="tabella-rosa">
        <thead><tr><th>Nome</th><th>Controparte</th><th>Direzione</th><th>Tipo</th><th>Costo prestito</th><th>Costo riscatto</th><th>Stipendio annuo</th><th>Quota %</th><th></th></tr></thead>
...
function rigaPrestitoGiocatore(p, i) {
  ...
  return `<tr data-i="${i}">
    ...
    <td><input class="pg-costo-prestito cifra" type="number" value="${p.costoPrestito || 0}" /></td>
    <td><input class="pg-costo-riscatto cifra" type="number" value="${p.costoRiscatto || 0}" /></td>
...
function renderStoricoStagioni() {
  ...
  if (storico.length === 0) {
    ...
    <div class="sezione">
      ...
      <p style="font-size:12px;color:var(--gesso-ombra)">Copia non modificabile del riassunto di ogni stagione chiusa — resta qui per sempre come archivio. Appare qui sotto non appena chiudi la prima stagione.</p>
  ...
  <div class="sezione">
    <h3>Registro bilanci passati</h3>
    <p style="font-size:12px;color:var(--gesso-ombra)">Copia non modificabile del riassunto di ogni stagione chiusa — resta qui per sempre come archivio.</p>
    <div class="tabella-scroll">
      <table class="tabella-rosa">
...
function agganciaEventi(tab) {
  ...
  if (tab === "anagrafica") {
    ...
    // Nessun evento da agganciare: la Rosa è solo un report, in sola lettura.
  } else if (tab === "mercato") {
    ...
    document.getElementById("btn-agg-prestito-giocatore").addEventListener("click", () => {
      ...
      stato.prestitiGiocatori.push({ nome: "", controparte: "", direzione: "In entrata", tipo: "Secco", costoPrestito: 0, costoRiscatto: 0, stipendioACarico: 0, quotaPercento: 100 });
...
function renderIstruzioni() {
  ...
  ${sezioneIstr("👕 Rosa calciatori", `
    Questa pagina è un <b>report in sola lettura</b>: mostra chi hai in squadra, ma non si modifica da qui — tutto passa dalla pagina Mercato (acquisti, cessioni, svincoli, rinnovi). Serve a evitare che i dati di un contratto cambino per sbaglio a stagione in corso.<br><br>
    ...
    <b>Costo</b>: quanto l'hai pagato. <b>Anno inizio/fine contratto</b>: fissati all'acquisto, cambiano solo con un Rinnovo — da lì si calcolano da soli "Anni rimanenti" e "Valore residuo".<br>
    ...
    <b>Quota %</b>: 100 se è tuo per tutta la stagione, 50 se entra/esce a metà, 0 se è fuori in prestito — questa resta fissata all'acquisto, per cambiarla serve rifare l'acquisto (svincolo + nuovo acquisto).<br><br>
    In fondo alla pagina trovi anche il <b>Registro modifiche</b>: ogni acquisto, cessione, svincolo e rinnovo viene segnato con data e ora, visibile anche all'ADMIN aprendo la squadra — per controllare che nessuna modifica sospetta sia stata fatta a stagione in corso.
    ${tabFasce}
  `)}
  ...
  ${sezioneIstr("🔁 Mercato — Acquisti, Cessioni, Svincoli, Rinnovi, Prestiti giocatori", `
    ...
    <b>Prestiti giocatori tra squadre</b>: diverso dagli altri — se lo prendi IN ENTRATA non aggiungerlo in Rosa con un costo vero (o mettilo a 0), il vero impatto lo scrivi qui. Per un prestito Secco basta "Costo prestito"; con Diritto/Obbligo di riscatto compili anche "Costo riscatto" (0 se gratis). Lo stipendio a carico funziona come in Rosa: importo annuo × quota (100/50/0%). Se a fine anno il riscatto/obbligo si esercita davvero, il giocatore resta tuo per sempre — registra un Acquisto con costo = costo riscatto e togli la riga da qui; con un prestito Secco invece torna al proprietario da solo.
  `)}
