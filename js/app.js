    ...
    <div class="griglia-2">
      ...
      <div class="campo">
        ...
        <div style="display:flex;align-items:center;gap:12px">
          <input type="number" id="f-stagione" value="${stato.stagioneCorrente}" style="width:100px;margin:0" />
          <span style="font-family:var(--font-cifre);font-size:20px;color:var(--ambra);font-weight:600">${stato.stagioneCorrente}/${String(stato.stagioneCorrente + 1).slice(-2)}</span>
        </div>
      </div>
      <div class="campo">
...
function renderRosa() {
  ...
  const sezioni = gruppi.map(({ chiave, etichetta }) => {
    const giocatori = stato.rosa.map((p, idx) => ({ p, idx })).filter(({ p }) => (p.gruppo || "Titolare") === chiave);
    const righe = giocatori.map(({ p, idx }) => rigaRosa(p, idx)).join("");
    return `
    <p style="font-size:13px;font-weight:600;color:var(--ambra);margin:16px 0 6px">${etichetta} — ${giocatori.length}</p>
  ...
  const titolari = stato.rosa.filter(p => (p.gruppo || "Titolare") === "Titolare");
  const comp = calc.composizioneRuoli(titolari);
  ...
  const inEntrata = (stato.prestitiGiocatori || []).filter(pg => pg.direzione === "In entrata");
  const sezioneEntrata = inEntrata.length === 0 ? "" : `
    <p style="font-size:13px;font-weight:600;color:var(--ok);margin:16px 0 6px">In prestito da altre squadre (in verde, non pesano su costo/ammortamento qui) — ${inEntrata.length}</p>
    <div class="tabella-scroll"><table class="tabella-rosa">
      <thead><tr><th>Ruolo</th><th>Nome</th><th>Costo</th><th>Inizio</th><th>Fine</th><th>Anni rim.</th><th>Stipendio</th><th>Quota%</th><th>Ammort.</th><th>Rinnovi</th><th>Val. res.</th><th>Sposta a</th></tr></thead>
      <tbody>${inEntrata.map(rigaRosaPrestitoEntrata).join("")}</tbody>
    </table></div>`;

  return `
  <div class="sezione">
    <h3>Rosa calciatori</h3>
    <p style="font-size:12px;color:var(--gesso-ombra)">Report — si modifica da Mercato. Sposta un giocatore tra Titolari/Under 21/Extra col menu a destra di ogni riga. Riga rossa = dato in prestito in uscita. Riga verde = preso in prestito da un'altra squadra (solo per vederlo, non è nella tua Rosa vera).</p>
...
function rigaRosaPrestitoEntrata(p) {
...
function renderPrestitiGiocatoriResoconto() {
  ...
  const righe = lista.map(p => {
    ...
    return `<tr><td>${p.nome}</td><td>${p.direzione}</td><td>${p.controparte || ""}</td><td class="cifra">${impatto.toFixed(1)}</td></tr>`;
  }).join("");
  return `
  <div class="sezione">
    <h3>Prestiti in corso (riepilogo)</h3>
    <p style="font-size:12px;color:var(--gesso-ombra)">Solo per vedere chi è via/in più mentre guardi la Rosa — si modifica da Mercato.</p>
    <div class="tabella-scroll"><table class="tabella-rosa">
      <thead><tr><th>Nome</th><th>Direzione</th><th>Controparte</th><th>Impatto tot.</th></tr></thead>
      <tbody>${righe}</tbody>
    </table></div>
  </div>`;
}
  ...
  <div class="sezione">
    ...
    <div class="tabella-scroll">
      <table class="tabella-rosa">
        <thead><tr><th>Nome</th><th>Controparte</th><th>Direzione</th><th>Tipo</th><th>Costo prestito</th><th>Costo riscatto</th><th>Stipendio annuo</th><th>Quota %</th><th></th><th></th></tr></thead>
...
function rigaPrestitoGiocatore(p, i) {
  ...
  return `<tr data-i="${i}">
    ...
    <td><select class="pg-direzione" style="min-width:110px">${opz(direzioni, p.direzione)}</select></td>
    <td><select class="pg-tipo" style="min-width:150px">${opz(tipi, p.tipo)}</select></td>
    ...
    <td>${puoRiscattare ? `<button class="btn-piccolo pg-conferma-riscatto" style="margin:0">Conferma riscatto</button>` : ""}</td>
...
function renderIstruzioni() {
  ...
  ${sezioneIstr("👕 Rosa calciatori", `
    ...
    <b>Colori</b>: riga <b>rossa</b> = quel giocatore è dato in prestito in uscita (resta comunque tuo, stipendio/ammortamento continuano). Riga <b>verde</b> = preso in prestito da un'altra squadra — appare qui solo per vederlo tutto insieme, ma non è nella tua Rosa vera: non ha costo/contratto/ammortamento con te, l'unico numero vero è lo stipendio a tuo carico (da Mercato → Prestiti giocatori).
    ${tabFasce}
  `)}
