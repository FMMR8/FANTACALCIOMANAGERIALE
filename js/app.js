...
function renderScadenzeRinnovo() {
  ...
  const righe = inScadenza.map(({ p, idx }) => {
    if (p.rinnovoInSospeso) {
      return `<tr><td colspan="6" style="color:var(--oro-chiaro)">${p.nome} — rinnovo di +${p.rinnovoInSospeso.durata} già confermato, parte dalla prossima stagione</td></tr>`;
    }
    return `
    <tr>
