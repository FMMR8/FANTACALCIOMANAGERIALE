...
function renderStoricoStagioni() {
  ...
  <div class="sezione">
    ...
    <div class="tabella-scroll">
      <table class="tabella-rosa">
        <thead><tr><th>Anno</th><th>Variazione Capitale</th><th>Cap. Provv. Prossima</th><th>Campionato</th><th>Coppa</th><th>Champions</th><th>Stadio</th></tr></thead>
        <tbody>${righe}</tbody>
      </table>
    </div>
  </div>`;
...
async function renderAltreSquadre() {
  ...
  // Classifica finanziaria: si basa sulle stagioni GIÀ CHIUSE (dati definitivi dello storico),
  // non su quelli in corso — che sarebbero fuorvianti finché non tutti hanno inserito premi e partite.
  // Mostra l'ultima stagione che risulta chiusa da almeno una squadra; chi non l'ha ancora chiusa
  // semplicemente non compare, e si aggiunge da solo quando la chiude.
  const conStorico = squadre.filter(s => s.nickname !== "_presidente" && (s.storicoStagioni || []).length > 0);
  let bloccoClassifica = "";
  if (conStorico.length === 0) {
    bloccoClassifica = `
    <div class="sezione">
      <h3>Classifica finanziaria</h3>
      <p style="font-size:13px;color:var(--testo-soft);margin:0">Apparirà qui dalla seconda stagione, quando le squadre inizieranno a chiudere la prima.</p>
    </div>`;
  } else {
    // L'anno più recente che qualcuno ha chiuso
    const annoUltimo = Math.max(...conStorico.map(s => Math.max(...s.storicoStagioni.map(h => h.anno))));
    const inClassifica = conStorico
      .map(s => {
        const riga = (s.storicoStagioni || []).find(h => h.anno === annoUltimo);
        return riga ? { nome: s.nomeSquadra || s.nickname, nickname: s.nickname, cap: riga.capitaleProvvisorio, variaz: riga.variazioneCapitale } : null;
      })
      .filter(Boolean)
      .sort((a, b) => b.cap - a.cap);

    const righe = inClassifica.map((s, i) => {
      const mia = s.nickname === nickCorrente;
      const colore = s.variaz >= 0 ? "var(--ok)" : "var(--rosso-cartellino)";
      return `<tr${mia ? ' style="background:rgba(209,163,44,0.10)"' : ""}>
        <td>${i + 1}°</td>
        <td>${s.nome}${mia ? " <span style='color:var(--oro-chiaro);font-size:11px'>(tu)</span>" : ""}</td>
        <td class="cifra">${s.cap.toFixed(1)}</td>
        <td class="cifra" style="color:${colore}">${s.variaz >= 0 ? "+" : ""}${s.variaz.toFixed(1)}</td>
      </tr>`;
    }).join("");

    const mancanti = squadre.filter(s => s.nickname !== "_presidente").length - inClassifica.length;
    bloccoClassifica = `
    <div class="sezione">
      <h3>Classifica finanziaria — stagione ${formattaStagione(annoUltimo)}</h3>
      <p style="font-size:12px;color:var(--testo-soft)">Dati definitivi di fine stagione.${mancanti > 0 ? ` Le squadre che non hanno ancora chiuso la ${formattaStagione(annoUltimo)} compariranno appena lo faranno.` : ""}</p>
      <div class="tabella-scroll"><table class="tabella-rosa">
  ...
  el.innerHTML = `
    <div class="sezione">
      ...
      <p style="font-size:12px;color:var(--testo-soft)">Tocca una squadra per vederne la rosa.</p>
      <div style="display:flex;flex-direction:column;gap:8px">
        ${altre.length === 0
          ? `<p style="color:var(--testo-soft);font-size:13px;margin:0">Non ci sono ancora altre squadre create.</p>`
...
async function renderRosaAltraSquadra(nickname, nomeVisualizzato) {
  ...
  const sezioni = gruppi.map(({ chiave, etichetta }) => {
    ...
    return `
    <p style="font-size:13px;font-weight:600;color:var(--oro-chiaro);margin:14px 0 6px">${etichetta} — ${giocatori.length}</p>
    <div class="tabella-scroll"><table class="tabella-rosa">
      ...
      <tbody>${righe || `<tr><td colspan="4" style="text-align:center;color:var(--testo-soft);font-size:12px;padding:8px">— nessuno —</td></tr>`}</tbody>
    </table></div>`;
  }).join("");
  el.innerHTML = `
  <div class="sezione">
