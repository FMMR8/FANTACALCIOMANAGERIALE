...
let squadraControparteCache = null; // { nickname, rosa } dell'ultima squadra scelta come Controparte in Prestiti giocatori, per pescarne i giocatori veri
...
function renderPrestitiGiocatoriResoconto() {
  ...
  <div class="sezione">
    ...
    <p style="font-size:12px;color:var(--gesso-ombra)">Fisso = quello che paghi/incassi comunque. Variabile = riscatto (conta già nel bilancio solo con Obbligo). Solo per vedere tutto insieme mentre guardi la Rosa — si modifica da Mercato.</p>
    <div class="tabella-scroll"><table class="tabella-rosa">
...
function opzioniStagioni(selezionata) {
  return Array.from({ length: 21 }, (_, i) => 2020 + i)
...
function renderMercato() {
  ...
  const campoControparteNuovo = altreSquadre.length > 0
    ? `<select id="pg-nuovo-controparte"><option value="">— scegli —</option>${altreSquadre.map(s => `<option value="${s.nickname}">${s.nomeSquadra}</option>`).join("")}</select>`
    : `<input type="text" id="pg-nuovo-controparte" placeholder="Nome squadra..." />`;
  ...
  <div class="sezione">
    ...
    <p style="font-size:12px;color:var(--gesso-ombra)">Negativo se paghi tu (di solito entrata), positivo se incassi/risparmi (di solito uscita). Riscatto conta solo con Obbligo. Una volta registrato non si modifica più — se sbagli, cancella (✕) e ricrea.</p>
    <div class="griglia-2">
    ...
    <div class="griglia-3">
      ...
      <div class="campo"><label>Tipo</label><select id="pg-nuovo-tipo"><option value="Secco">Secco</option><option value="Diritto di riscatto">Diritto di riscatto</option><option value="Obbligo di riscatto">Obbligo di riscatto</option></select></div>
    </div>
    <div class="griglia-3">
      ...
      <div class="campo"><label>Stipendio annuo</label><input type="number" id="pg-nuovo-stipendio" value="0" /></div>
    </div>
  ...
  if (tab === "anagrafica") {
    ...
    if (selControparte && selControparte.tagName === "SELECT") {
      selControparte.addEventListener("change", async () => {
        ...
        try {
          ...
          squadraControparteCache = { nickname, rosa: rosaAltro };
    ...
    if (selGiocatoreControparte) {
      selGiocatoreControparte.addEventListener("change", () => {
        ...
        if (idx === "" || !squadraControparteCache) return;
        const player = squadraControparteCache.rosa[parseInt(idx)];
        if (!player) return;
    ...
    document.getElementById("btn-agg-prestito-giocatore").addEventListener("click", () => {
      ...
      if (!nome) { alert("Inserisci il nome del giocatore."); return; }
      const controparteEl = document.getElementById("pg-nuovo-controparte");
      // Se il campo è una tendina (valore = nickname), traduco nel nome squadra vero per mostrarlo bene;
      // se è testo libero (nessuna squadra ancora caricata), uso semplicemente quello che hai scritto.
      let controparte = controparteEl.value.trim();
      if (controparteEl.tagName === "SELECT" && elencoAltreSquadreCache) {
        const trovata = elencoAltreSquadreCache.find(s => s.nickname === controparteEl.value);
        if (trovata) controparte = trovata.nomeSquadra;
      }
      const nuovo = {
...
function renderIstruzioni() {
  ...
  ${sezioneIstr("🔁 Mercato — Acquisti, Cessioni, Svincoli, Rinnovi, Prestiti giocatori", `
    ...
    <b>Prestiti giocatori tra squadre</b>: si registrano nel modulo, poi restano in un elenco di sola lettura come tutti gli altri — se sbagli, cancella (✕) e ricrea, non si modifica direttamente. Se scegli IN ENTRATA, seleziona prima la squadra Controparte poi il "Giocatore della loro rosa": nome, ruolo e stipendio vero si riempiono da soli (stipendio già negativo, perché lo pagheresti tu) — comodo e senza errori, ma resta possibile scrivere tutto a mano se preferisci. Non aggiungerlo in Rosa con un costo vero (o mettilo a 0), il vero impatto lo scrivi qui. "Costo prestito": <b>negativo</b> se lo paghi tu per prenderlo (di solito IN ENTRATA), <b>positivo</b> se lo incassi tu per darlo (di solito IN USCITA). Per un prestito Secco basta questo, lascia "Costo riscatto" a 0 (non conta comunque). Con <b>Obbligo</b> di riscatto, "Costo riscatto" conta subito nel bilancio (mostrato nella colonna "Fisso"). Con <b>Diritto</b> di riscatto invece resta nella colonna "Variabile" — non conta ancora, finché non premi "Conferma riscatto" (il tasto compare solo su Diritto/Obbligo): a quel punto, se era IN USCITA diventa una vera Cessione, se era IN ENTRATA il giocatore entra davvero in Rosa. "Stipendio annuo" è il vero stipendio del giocatore (o la parte concordata): <b>negativo</b> se lo paghi tu (di solito IN ENTRATA), <b>positivo</b> se lo paga l'altra squadra ed è un risparmio per te (di solito IN USCITA) — con "Quota %" a 100/50/0 in base a quanta stagione copre. Con un prestito Secco il giocatore torna al proprietario da solo a fine prestito.<br>
