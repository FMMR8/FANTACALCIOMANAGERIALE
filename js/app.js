  ...
  <div class="sezione">
    ...
    <details style="margin-top:14px">
      ...
      <div style="margin-top:8px">
        <p style="font-size:12px;color:var(--gesso-ombra)">Una riga per giocatore, separando con virgola: <b>Ruolo,Nome,Costo,AnnoInizio,Durata</b> (quota resta 100%). Esempio: <code>A,Thuram,136,2026,4</code> (Durata in anni: 1, 2, 3...)</p>
        <textarea id="asta-import-testo" rows="8" style="width:100%;font-family:var(--font-cifre);font-size:12px;padding:10px;border-radius:8px;background:rgba(247,250,247,0.06);color:var(--gesso);border:1px solid rgba(247,250,247,0.2)" placeholder="P,Di Gennaro,1,2026,3&#10;D,Bartesaghi,7,2026,5&#10;..."></textarea>
  ...
  if (tab === "anagrafica") {
    ...
    document.getElementById("btn-import-asta").addEventListener("click", () => {
      ...
      righeTesto.forEach((riga, idx) => {
        ...
        const [ruoloRaw, nome, costoRaw, annoInizioRaw, durataRaw] = parti;
        ...
        const durata = parseInt(durataRaw);
        if (!ruolo || !nome || isNaN(costo) || isNaN(annoInizio) || isNaN(durata) || durata < 1) { righeSaltate.push(idx + 1); return; }
        const annoFine = annoInizio + durata - 1;
        const id = nuovoId();
...
async function renderRosaAltraSquadra(nickname, nomeVisualizzato) {
  ...
  const sezioni = gruppi.map(({ chiave, etichetta }) => {
    ...
    const righe = giocatori.map(p => `
      <tr><td style="text-align:center">${p.ruolo}</td><td style="text-align:center">${p.nome}</td><td style="text-align:center">${calc.anniRimanenti(p, stagioneAltro)}</td></tr>`).join("");
    return `
    <p style="font-size:13px;font-weight:600;color:var(--ambra);margin:14px 0 6px">${etichetta} — ${giocatori.length}</p>
    <div class="tabella-scroll"><table class="tabella-rosa">
      <thead><tr><th style="text-align:center">Ruolo</th><th style="text-align:center">Nome</th><th style="text-align:center">Anni rimanenti</th></tr></thead>
...
function renderIstruzioni() {
  ...
  ${sezioneIstr("👕 Rosa calciatori", `
    ...
    <b>Stipendio</b> = fascia di costo × 1,10 per ogni rinnovo × quota stagione.<br>
