import * as calc from "./calc.js?v=202609091214";
import {
...
} from "./storage.js?v=202609091214";
...
function graficoLinee(serie, { altezza = null, conLegenda = false } = {}) {
  ...
  const L = stretto ? 38 : 44, R = 12, T = 10, B = 24;
  const W = stretto ? 330 : 620;
  const H = altezza || (stretto ? 110 : 130);
  ...
  const griglia = tacche.map(v => `
    ...
    <text x="${L - 8}" y="${(py(v) + 4).toFixed(1)}" text-anchor="end"
          font-size="10" fill="#8a8f9a" font-family="monospace">${v.toFixed(0)}</text>`).join("");
  ...
  const etichetteX = anni.map((a, i) => {
    ...
    return `<text x="${px(a).toFixed(1)}" y="${H - 8}" text-anchor="${ancora}" font-size="10" fill="#8a8f9a">${formattaStagione(a)}</text>`;
  }).join("");
  ...
  const linee = serie.map(s => {
    ...
    const punti = ordinati.map(p => `<circle cx="${px(p.x).toFixed(1)}" cy="${py(p.y).toFixed(1)}" r="3" fill="${s.colore}" />`).join("");
    return `<path d="${d}" fill="none" stroke="${s.colore}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" />${punti}`;
  }).join("");
...
function renderStoricoStagioni() {
  ...
  return `
  ${(() => {
    // Grafico dell'andamento del capitale: solo stagioni già chiuse, mai quella in corso.
    const storico = (stato.storicoStagioni || []).slice().sort((a, b) => a.anno - b.anno);
    if (storico.length === 0) {
      return `<div class="sezione">
        <h3>Andamento capitale</h3>
        <p style="font-size:13px;color:var(--testo-soft);margin:0">Il grafico comparirà dalla seconda stagione, quando avrai chiuso la prima.</p>
      </div>`;
    }
    const serie = [{ nome: stato.nomeSquadra || nickCorrente, colore: "var(--oro-chiaro)", punti: storico.map(h => ({ x: h.anno, y: h.capitaleProvvisorio })) }];
    return `<div class="sezione">
      <h3>Andamento capitale</h3>
      <p style="font-size:12px;color:var(--testo-soft)">Capitale a fine di ogni stagione conclusa.</p>
      ${graficoLinee(serie.map(s => ({ ...s, colore: "#E8C766" })))}
    </div>`;
  })()}
  ...
  const bloccoFinanze = `
    ...
    ${(() => {
      ...
      const serie = conStorico
        .map((s, i) => ({
          nome: nomeDi(s),
          colore: COLORI_SQUADRE[i % COLORI_SQUADRE.length],
          punti: (s.storicoStagioni || []).slice().sort((a, b) => a.anno - b.anno)
            .map(h => ({ x: h.anno, y: h.capitaleProvvisorio })),
        }))
        .filter(s => s.punti.length > 0);
      if (serie.length === 0) return "";
      return `<div class="sezione">
        <h3>Andamento capitale delle squadre</h3>
        <p style="font-size:12px;color:var(--testo-soft)">Capitale a fine di ogni stagione conclusa, tutte le squadre a confronto.</p>
