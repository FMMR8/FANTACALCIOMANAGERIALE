...
function renderPrestitiGiocatoriResoconto() {
  ...
  const righe = lista.map(p => {
    const impatto = calc.impattoPrestitoGiocatore(p);
    return `<tr>
...
function renderMercato() {
  ...
  const campoControparteNuovo = altreSquadre.length > 0
    ? `<select id="pg-nuovo-controparte"><option value="">— scegli —</option>${altreSquadre.map(n => `<option value="${n}">${n}</option>`).join("")}</select>`
  ...
  <div class="sezione">
    ...
    <div class="tabella-scroll" style="margin-top:14px"><table class="tabella-rosa">
      <thead><tr><th>Nome</th><th>Direz.</th><th>Controparte</th><th>Tipo</th><th>Prestito</th><th>Riscatto</th><th>Stipendio</th><th>Impatto</th><th></th><th></th></tr></thead>
  ...
  if (tab === "anagrafica") {
    ...
    document.getElementById("btn-agg-prestito-giocatore").addEventListener("click", () => {
      ...
      const nuovo = {
        ...
        nome,
        controparte: document.getElementById("pg-nuovo-controparte").value.trim ? document.getElementById("pg-nuovo-controparte").value.trim() : document.getElementById("pg-nuovo-controparte").value,
