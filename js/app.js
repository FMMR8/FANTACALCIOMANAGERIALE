...
import { caricaSquadra, salvaSquadra, elencoSquadre, eliminaSquadra, creaProposta, elencoProposte, aggiornaProposta, eliminaProposta } from "./storage.js";
    ...
    document.querySelectorAll(".pgr-riscatto").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        if (p.direzione === "In uscita") {
          ...
          stato.rosa.push({ id, ruolo, nome: p.nome, costo: costoRiscattoAssoluto, annoInizio: stato.stagioneCorrente, annoFine, quotaStagione: 1, rinnovi: 0, gruppo: "Titolare" });
...
function renderMercatoSfoglia() {
  ...
  const rosaHtml = () => {
    ...
    const righe = mercatoSquadraScelta.rosa.map((p, idx) => {
      ...
      ${apertoQui ? `<tr><td colspan="4">${formModuloOfferta(p)}</td></tr>` : ""}`;
    }).join("");
    return `
    <div class="tabella-scroll" style="margin-top:12px"><table class="tabella-rosa">
      <thead><tr><th>Ruolo</th><th>Nome</th><th>Costo</th><th></th></tr></thead>
      <tbody>${righe}</tbody>
    </table></div>`;
  };
...
function formModuloOfferta(giocatoreAltrui) {
  ...
  <div class="sezione" style="margin:8px 0;padding:12px;background:rgba(255,255,255,.03)">
    ...
    <div class="griglia-2" id="of-prestito-wrap" hidden>
      <div class="campo"><label>Costo riscatto (se Diritto)</label><input type="number" id="of-riscatto" value="0" /></div>
      <div class="campo"><label>Stipendio annuo a tuo carico</label><input type="number" id="of-stipendio" value="0" min="0" /></div>
    </div>
    <div class="campo" id="of-scambio-wrap" hidden>
      ...
      <p style="font-size:11px;color:var(--gesso-ombra);margin:4px 0 0">Il "Prezzo" sopra diventa un eventuale conguaglio in soldi che paghi tu in aggiunta (0 se scambio alla pari).</p>
    </div>
...
function agganciaEventiMercatoSfoglia() {
  ...
  if (selTipo) {
    const aggiorna = () => {
      ...
      document.getElementById("of-prestito-wrap").hidden = !(t === "prestito_secco" || t === "prestito_riscatto");
  ...
  document.querySelectorAll(".of-invia").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      await creaProposta(nuovaProposta);
...
async function renderMercatoOfferte() {
  const tutte = await elencoProposte();
  const mie = tutte.filter(p => p.proponenteNickname === nickCorrente || p.proprietarioNickname === nickCorrente);
  const el = document.getElementById("contenuto-proposte");
  ...
  <div class="sezione">
    ...
    <div class="tabella-scroll"><table class="tabella-rosa">
      <thead><tr><th>Giocatore</th><th>Tipo</th><th>Con</th><th>Prezzo</th><th>Stato</th><th></th></tr></thead>
...
async function rispondiOfferta(id, azione, nuovoPrezzo) {
  ...
  const sonoProponente = p.proponenteNickname === nickCorrente;
  if (azione === "rifiuta") {
    await aggiornaProposta(id, { stato: "rifiutato", dataUltimaModifica: new Date().toISOString() });
  } else if (azione === "accetta") {
    await aggiornaProposta(id, { stato: "accettato", dataUltimaModifica: new Date().toISOString() });
  } else if (azione === "controfferta") {
    await aggiornaProposta(id, { prezzo: nuovoPrezzo, turnoDi: sonoProponente ? "proprietario" : "proponente", dataUltimaModifica: new Date().toISOString() });
  }
...
async function completaTrasferimento(id, proposta) {
  if (!proposta) { const tutte = await elencoProposte(); proposta = tutte.find(p => p.id === id); }
  if (!proposta) return;
  ...
  if (tipo === "prestito_secco" || tipo === "prestito_riscatto") {
    ...
    if (sonoProponente) {
      const annoFineInput = prompt(`Contratto per "${giocatore.nome}" fino a che anno?`, String(stato.stagioneCorrente + 3));
      const annoFine = parseInt(annoFineInput) || stato.stagioneCorrente;
      const id2 = nuovoId();
      stato.rosa.push({ id: id2, ruolo: giocatore.ruolo, nome: giocatore.nome, costo: prezzo, annoInizio: stato.stagioneCorrente, annoFine, quotaStagione: 1, rinnovi: 0, gruppo: "Titolare" });
      stato.acquistiFuoriAsta = stato.acquistiFuoriAsta || [];
    ...
    if (sonoProponente) {
      ...
      const annoFineInput = prompt(`Contratto per "${giocatore.nome}" (ricevuto in scambio) fino a che anno?`, String(stato.stagioneCorrente + 3));
      const annoFine = parseInt(annoFineInput) || stato.stagioneCorrente;
      const id2 = nuovoId();
      stato.rosa.push({ id: id2, ruolo: giocatore.ruolo, nome: giocatore.nome, costo: prezzo, annoInizio: stato.stagioneCorrente, annoFine, quotaStagione: 1, rinnovi: 0, gruppo: "Titolare" });
      stato.acquistiFuoriAsta = stato.acquistiFuoriAsta || [];
      ...
      const annoFineInput = prompt(`Contratto per "${giocatoreScambio.nome}" (ricevuto in scambio) fino a che anno?`, String(stato.stagioneCorrente + 3));
      const annoFine = parseInt(annoFineInput) || stato.stagioneCorrente;
      const id2 = nuovoId();
      stato.rosa.push({ id: id2, ruolo: giocatoreScambio.ruolo, nome: giocatoreScambio.nome, costo: 0, annoInizio: stato.stagioneCorrente, annoFine, quotaStagione: 1, rinnovi: 0, gruppo: "Titolare" });
      stato.acquistiFuoriAsta = stato.acquistiFuoriAsta || [];
  ...
  await aggiornaProposta(id, { ...aggiornamento, dataUltimaModifica: new Date().toISOString() });
  programmaSalvataggio();
  renderMercatoOfferte();
  aggiornaBadgeProposte();
}
...
async function renderMercatoCompletate() {
  ...
  <div class="sezione">
    ...
    <div class="tabella-scroll"><table class="tabella-rosa">
      <thead><tr><th>Da</th><th>A</th><th>Giocatore</th><th>Tipo</th><th>Costo</th></tr></thead>
      <tbody>${righe}</tbody>
    </table></div>`}
  </div>`;
}
