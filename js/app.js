...
function renderAnagrafica() {
  ...
  <div class="sezione">
    ...
    <div class="griglia-2">
      ...
      <div class="campo">
        <label>Capitale speso in asta (mln)</label>
        <p style="font-family:var(--font-cifre);font-size:18px;color:var(--ambra);margin:8px 0 0">${(stato.acquistiAsta || []).reduce((s, a) => s + (a.costo || 0), 0).toFixed(1)}</p>
        <p style="font-size:11px;color:var(--gesso-ombra);margin:2px 0 0">Calcolato da solo sommando gli "Acquisti in asta" in Mercato — se ne annulli uno, questo numero si aggiorna da solo.</p>
      </div>
    </div>
  </div>`;
}
...
function renderMercato() {
  ...
  <div class="sezione">
    ...
    <p style="font-size:11px;color:var(--gesso-ombra);margin:-6px 0 10px">Scegli sopra per caricare da solo ruolo e costo — oppure scrivi tutto a mano sotto.</p>
    <div class="griglia-3">
  ...
  if (tab === "anagrafica") {
    ...
    if (selGiocatoreFa) {
      selGiocatoreFa.addEventListener("change", () => {
        ...
        document.getElementById("fa-prezzo").value = (player.costo || 0).toFixed(1);
      });
    }
...
async function renderRosaAltraSquadra(nickname, nomeVisualizzato) {
  ...
  const sezioni = gruppi.map(({ chiave, etichetta }) => {
    ...
    const righe = giocatori.map(p => `
      <tr><td>${p.ruolo}</td><td>${p.nome}</td><td class="cifra">${calc.anniRimanenti(p, stagioneAltro)}</td></tr>`).join("");
    return `
    <p style="font-size:13px;font-weight:600;color:var(--ambra);margin:14px 0 6px">${etichetta} — ${giocatori.length}</p>
    <div class="tabella-scroll"><table class="tabella-rosa">
      <thead><tr><th>Ruolo</th><th>Nome</th><th>Anni rimanenti</th></tr></thead>
      <tbody>${righe || `<tr><td colspan="3" style="text-align:center;color:var(--gesso-ombra);font-size:12px;padding:8px">— nessuno —</td></tr>`}</tbody>
    </table></div>`;
  }).join("");
  el.innerHTML = `
  <div class="sezione">
...
function renderIstruzioni() {
  ...
  ${sezioneIstr("👕 Rosa calciatori", `
    Report: i dati di ogni giocatore (costo, contratto, quota) si modificano solo da Mercato — l'unica cosa che fai qui è spostare un giocatore tra <b>Titolare / Under 21 / Extra</b> col menu a destra di ogni riga. Dentro ogni gruppo i giocatori sono ordinati P, D, C, A.<br><br>
