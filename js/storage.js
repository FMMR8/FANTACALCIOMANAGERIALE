...
// ============================================================
import { firebaseConfig } from "./firebase-config.js?v=202609102141";
import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";
import {
  getFirestore, doc, getDoc, setDoc, collection, getDocs, deleteDoc, addDoc, updateDoc, query, orderBy, limit,
...
// ---------- Notizie della lega ----------
// Diario condiviso: ogni squadra ci scrive gli eventi che la riguardano (trasferimenti conclusi,
// stadio, trofei, record, capitale in rosso) e tutti li leggono. Stessa logica delle proposte:
// un posto neutro dentro la lega, nessuno scrive nel documento di un altro.
export async function creaNotizia(legaId, notizia) {
  const ref = await addDoc(collection(db, LEGHE_COLLECTION, legaId, "notizie"), notizia);
  return ref.id;
}

export async function elencoNotizie(legaId, quante = 120) {
  // Chiedo al server solo le più recenti: dopo qualche stagione le notizie diventano centinaia
  // e scaricarle tutte a ogni apertura rallenterebbe la pagina per niente.
  const q = query(
    collection(db, LEGHE_COLLECTION, legaId, "notizie"),
    orderBy("data", "desc"),
    limit(quante),
  );
  const snap = await getDocs(q);
  const notizie = [];
  snap.forEach((d) => notizie.push({ id: d.id, ...d.data() }));
  return notizie;
}

// ---------- Oscar di fine stagione ----------
// Un documento per ogni votante e stagione: "{anno}__{nickname}". Usare un id prevedibile
// invece di uno casuale evita doppioni se qualcuno vota due volte per sbaglio.
export async function salvaVotoOscar(legaId, anno, nickname, voti) {
  await setDoc(doc(db, LEGHE_COLLECTION, legaId, "oscar", `${anno}__${nickname}`), {
    anno, nickname, voti, data: new Date().toISOString(),
  });
}

export async function elencoVotiOscar(legaId) {
  const snap = await getDocs(collection(db, LEGHE_COLLECTION, legaId, "oscar"));
  const voti = [];
  snap.forEach((d) => voti.push({ id: d.id, ...d.data() }));
  return voti;
}
