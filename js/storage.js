...
export async function elencoNotizie(legaId) {
  const snap = await getDocs(collection(db, LEGHE_COLLECTION, legaId, "notizie"));
  const notizie = [];
  snap.forEach((d) => notizie.push({ id: d.id, ...d.data() }));
  // Più recenti in alto
  return notizie.sort((a, b) => (b.data || "").localeCompare(a.data || ""));
}
